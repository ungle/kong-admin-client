...
import io.github.ungle.kong.client.model.plugins.security.BotDetectionConfig;
...
public class BotDetectionPluginModelTest {
	...
	public void createPlugin() throws Exception {
		...
		String pluginName = InnerPluginName.BOT_DETECTION.getPluginName();
		request.setName(pluginName);
		request.setService(new IdNameRelation(serviceId));
		request.setConfig(BotDetectionConfig.builder().withAllow(Collections.singletonList("172.1.1.33")).build());
